resource "aws_security_group" "internal_ssh" {
  ...
  merge(
    map(
       "Name", "${format("%s-internal-ssh",module.gitlab_label.name)}",
       "Environment", "module.gitlab_label.stage"
    )
  )}"
...
resource "aws_security_group" "external_ssh" {
  ...
  merge(
    map(
       "Name", "${format("%s-external-ssh",module.gitlab_label.name)}",
       "Environment", "module.gitlab_label.stage"
    )
  )}"
...
resource "aws_security_group" "gitlab_alb" {
  ...
  merge(
    map(
       "Name", "${format("%s-gitlab-alb",module.gitlab_label.name)}",
       "Environment", "module.gitlab_label.stage"
    )
  )}"
...
resource "aws_security_group" "internal_gitlab" {
  ...
  merge(
    map(
       "Name", "${format("%s-internal-gitlab",module.gitlab_label.name)}",
       "Environment", "module.gitlab_label.stage"
    )
  )}"
